...
function generateData(nodeCount) {
  ...
  // Start with a connected ring to ensure one connected component
  for (let i = 0; i < nodeCount; i++) {
  ...
  // Add extra random links to create a mesh-like structure
  for (let i = 0; i < nodeCount * 2; i++) {
    const a = Math.floor(Math.random() * nodeCount);
    let b = Math.floor(Math.random() * nodeCount);
    while (b === a || links.find(l => (l.source === `Node ${a}` && l.target === `Node ${b}`) || (l.source === `Node ${b}` && l.target === `Node ${a}`))) {
